<?php
	...
	header("Content-Type: text/event-stream\n\n");
 	...
 	echo 'data: '.$message."\n\n";  
  	...
  	$fileContents = file_get_contents('preview.pdf');
  	if ( $homeID < 144 ){
  		$pathVar = '/Inspection_Notices/SRP/'.date('Y').'/'.$homeID.'_'.$_GET['id'].'.pdf';
	}
	else if ( $homeID < 287 ){
		$pathVar = '/Inspection_Notices/SRSQ/'.date('Y').'/'.$homeID.'_'.$_GET['id'].'.pdf';
	}
	...
	$message  = "Seneding email...Please wait...";
		echo 'data: '.$message."\n\n";  
  		ob_end_flush();
  		flush();
