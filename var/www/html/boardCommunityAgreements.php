    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <h1><strong>Community Agreements</strong><small> - <?php echo $_SESSION['hoa_community_name']; ?></small></h1>
              ...
              <div class="box">
                ...
                <div class="box-header">
                  ...
                  <div class="box-tools pull-right">
                    ...
                    <a type="button" href="https://hoaboardtime.com/boardSendAgreement.php" class="btn bg-teal btn-sm">Send New Agreement</a>
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                      ...
                      <?php
                        ...
                        $header[] = "Access-Token:3AAABLblqZhBWF9BYTpVk2qiLdux9HoMp6296MnQhdvuw5sR-wRF84ZkKs3rUG6GDbSI8MVYE2-Kgabac7qiVa1FqAytq957r";
                                        
                        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
                        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                        curl_setopt($ch, CURLOPT_URL, 'https://api.na1.echosign.com/api/rest/v5/megaSigns/');
                        curl_setopt($ch, CURLOPT_HTTPHEADER, $header);
                                        
                        $result = curl_exec($ch);
                        $obj = json_decode($result);

                        foreach ($obj->megaSignList as $key) {
                                        
                          curl_setopt($ch, CURLOPT_URL, "https://api.na1.echosign.com/api/rest/v5/megaSigns/".$key->megaSignId."/agreements");
                                            
                          $result1 = curl_exec($ch);
                          $obj1 = json_decode($result1);

                          foreach ($obj1->megaSignChildAgreementList as $key1) {
                                            
                            curl_setopt($ch, CURLOPT_URL, "https://api.na1.echosign.com/api/rest/v5/agreements/".$key1->agreementId);
                                                
                            $result2 = curl_exec($ch);
                            $obj2 = json_decode($result2);

                            $result100 = pg_query("SELECT lastname, firstname, community_id, home_id FROM hoaid WHERE email='".$obj2->participantSetInfos[0]->participantSetMemberInfos[0]->email."'");
                                                
                            if($result100)
                            {
                              
                              $row100 = pg_fetch_assoc($result100);

                              $home_id = $row100['home_id'];
                              $hoa_id = $row100['firstname'];
                              $hoa_id .= " ";
                              $hoa_id .= $row100['lastname'];
                              $community_id = $row100['community_id'];
                                              
                              $result100 = pg_query("SELECT address1 FROM homeid WHERE home_id=".$home_id);
                              $row100 = pg_fetch_assoc($result100);
                              $home_id = $row100['address1'];

                            }

                            if(@$community_id == $_SESSION['hoa_community_id'])
                            {    
                              $aDate = date('Y-m-d', strtotime($obj2->events[0]->date));

                              echo "<tr><td>".$home_id."</td><td>".$hoa_id."</td><td>".$obj2->participantSetInfos[0]->participantSetMemberInfos[0]->email."</td><td>".$obj2->name."</td><td>".$aDate."</td><td>".$obj2->status."</td><td>";
                                                    
                              if($obj2->status == 'SIGNED')
                                echo $obj2->events[4]->date;
