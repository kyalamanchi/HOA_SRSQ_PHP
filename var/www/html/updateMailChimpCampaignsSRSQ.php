<?php
	...
	foreach ($result->campaigns as $campaigns) {
		...
		$query = "INSERT INTO COMMUNITY_CAMPAIGNS (campaign_id,community_id,create_time,status,emails_sent,send_time,content_type,recipients_list_id,opens,unique_opens,open_rate,clicks,subscriber_clicks,click_rate,title,reply_to,updated_on,updated_by) VALUES('".$campaigns->id."',2,'".$campaigns->create_time."','".$campaigns->status."',".$campaigns->emails_sent.",".$sendTime.",'".$campaigns->content_type."','".$campaigns->recipients->list_id."',".$campaigns->report_summary->opens.",".$campaigns->report_summary->unique_opens.",".round($campaigns->report_summary->open_rate*100,2).",".$campaigns->report_summary->clicks.",".$campaigns->report_summary->subscriber_clicks.",".round($campaigns->report_summary->click_rate*100,2).",'".$campaigns->settings->title."','".$campaigns->settings->reply_to."','".date('Y-m-d H:i:s')."',401) ON CONFLICT(campaign_id) DO UPDATE SET opens=".$campaigns->report_summary->opens.",unique_opens=".$campaigns->report_summary->unique_opens.",open_rate=".round($campaigns->report_summary->open_rate*100,2).",clicks=".$campaigns->report_summary->clicks.",subscriber_clicks=".$campaigns->report_summary->subscriber_clicks.",click_rate=".round($campaigns->report_summary->click_rate*100,2).",updated_on='".date('Y-m-d H:i:s')."',updated_by=401"	;
		}
		else {
			if ( (($campaigns->send_time) == "") ){
				$sendTime = "NULL";
