...
{
function ImprovedTable($header, $data,$currentChargesTotal2,$currentPaymentsTotal2,$homeID,$connection,$zipInfo,$stateInfo,$cityInfo)
{
    ...
    $z = "SELECT * FROM COMMUNITY_INFO WHERE COMMUNITY_ID = 1";
    $g = pg_query($z);
...
$homeQuery = "SELECT home_id FROM HOMEID WHERE COMMUNITY_ID = 1  ORDER BY HOME_ID";
$homeQueryResult = pg_query($homeQuery);
$homeIDSArray  = array();

while ($row  = pg_fetch_row($homeQueryResult)) {
    $homeIDSArray[$row[0]] = 1;
}
foreach ($homeIDSArray as $homeDS => $value) {
$currentChargesTotal = 0;
$currentPaymentsTotal = 0;
$pdf = new PDF();
...
$pdf->SetFont('Arial','',6);
$pdf->AddPage();
$pdf->ImprovedTable($header,$data,$currentChargesTotal,$currentPaymentsTotal,$homeDS,$connection,$zipInfo,$stateInfo,$cityInfo);
...
curl_setopt($ch, CURLOPT_HTTPHEADER, array('Authorization: Bearer n-Bgs_XVPEAAAAAAAAEQYgvfkzJWzxx59jqgvKQeXbtsYt-eXdZ6BNRYivEGKVGB','Content-Type:application/octet-stream','Dropbox-API-Arg: {"path": "/Billing_Statements/SRP/'.date('Y').'/'.$finalHOAID.'.zip'.'","mode": "overwrite","autorename": false,"mute": false}'));
