...
<div class="container">
      ...
      <input type="button" name="Current Date" value="Today Transactions" oncancel="buttonPressed()">
        ...
        <table id="example" class="table table-striped" cellspacing="0" width="100%" style="font-size: 16px;">
                               ...
                               date_default_timezone_set('America/Los_Angeles');
                                ...
                                if ($dbconn3) {
                                    $query = "select hoa_id,home_id from hoaid where community_id=1";
                                    ...
                                    $bankTransactions = "SELECT bank_transaction_id,hoa_id FROM current_payments WHERE community_id=1 AND bank_transaction_id IS NOT NULL";
                ...
                while ($row = pg_fetch_row($bankTransactionsResult)) {
                    $bankData[$row[0]]  = $row[1];
                               ...
                               $paymentTransactionIDS = array();
                                if ( !$_GET['date']) {
                                $url = 'https://api.forte.net/v3/organizations/org_335357/locations/loc_193771/transactions?filter=start_received_date+eq+'.$d.'+and+end_received_date+eq+'.$date.'';
                                }
                                else {
                                    $url = 'https://api.forte.net/v3/organizations/org_335357/locations/loc_193771/transactions?filter=received_date eq '.$date.'&page_size='.$result['number_results'];
        ...
        {
            $url = 'https://api.forte.net/v3/organizations/org_335357/locations/loc_193771/transactions?filter=start_received_date+eq+'.$d.'+and+end_received_date+eq+'.$date.'&page_size='.$result['number_results'];
        }
            $ch = curl_init($url);
            curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
            curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type:application/json','X-Forte-Auth-Organization-Id:org_335357','Authorization:Basic NjYxZmM4MDdiZWI4MDNkNTRkMzk5MjUyZjZmOTg5YTY6NDJhNWU4ZmNjYjNjMWI2Yzc4N2EzOTY2NWQ4ZGMzMWQ='));
                    ...
                    if ($bankData[$paymentTransactionIDS[$val]]){
                        ...
                        if ( $hoaIDS[$val] != -1) {
                           $payID = $hoaIDS[$val].$homeIDS[$val];
                            ...
                            $url  = 'https://api.forte.net/v3/organizations/org_335357/locations/loc_193771/transactions/'.$paymentTransactionIDS[$val];
                            $ch = curl_init($url);
                                curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
                                curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type:application/json','X-Forte-Auth-Organization-Id:org_335357','Authorization:Basic NjYxZmM4MDdiZWI4MDNkNTRkMzk5MjUyZjZmOTg5YTY6NDJhNWU4ZmNjYjNjMWI2Yzc4N2EzOTY2NWQ4ZGMzMWQ='));
                                curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
                            ...
                            print_r("An error occured.. Transaction id :".$paymentTransactionIDS[$val].nl2br("\n")."");
                        }
                        }
                        else {
                            print_r("Could not find matching home id...... Transaction id is : ".$paymentTransactionIDS[$val].nl2br("\n"));
                        }
                        }
    ...
    return $ipaddress;
        ...
        $scheduleUpdateQuery = "INSERT INTO SCHEDULED_JOBS(\"JOB_TITLE\",\"START_TIME\",\"RUN_BY\",\"IP_ADDRESS\") VALUES('SRP FORTE TRANSACTIONS','".date('Y-m-d H:i:s')."','MANUAL','".get_client_ip()."')";
