              ...
              if ( $data['id'] == $responseID ) {
                ...
                foreach ($data['pages'] as $page) {
                  ...
                  foreach ($page['questions'] as $question) {
                    ...
                    $answerID = $question['answers']['choice_id'];
                                            
                    foreach ($question['answers'] as $key ) {
                                                
                      $answerID = $key['choice_id'];
                    
                    }
                                            
                    $questionID = $question['id'];
                    $url2 = "https://api.surveymonkey.net/v3/surveys/".$surveyID."/pages/".$pageID."/questions/".$questionID;
                                            
                    $ch2 = curl_init($url2);
                                            
                    curl_setopt($ch2, CURLOPT_CUSTOMREQUEST , 'GET');
                    curl_setopt($ch2, CURLOPT_HTTPHEADER, array('Content-Type:application/json','Authorization:bearer i5mFSd4lF3PPe9OdOCz2KJvo56ZQaDxsLLMhHBjeHs9ezVl-zHaX-LFpI4UKkf29vcD1TfGN1D0qL4UWQUHNZFNbqhoRpHRAbNbW-48ilpWvIefaBjTvmVjlS4curfZf'));
                    ...
                    $questionText = $responseJSON['headings'];
                                            
                    foreach ($responseJSON['headings'] as $key ) {
                                                
                      $questionText = $key['heading'];

                    }
                                            
                    foreach ($responseJSON['answers'] as $choice) {
                      ...
                      foreach ($choice as $key ) {
                        ...
                        if ( $key['id'] == $answerID) {
                          ...
                          if ( $key['text'] ) {
                            ...
                            echo '<a class="list-group-item"><h5 class="list-group-item-heading" style="color: orange;"><b>Question : </b>'.$questionText.'</h5><p class="list-group-item-text" style="color: green;"><b>Response : </b>'.$key['text'].'</p></a>';

                          }
                          else {
                            
                            echo '<a class="list-group-item"><h5 class="list-group-item-heading" style="color: orange;"><b>Question : </b>'.$questionText.'</h5><p class="list-group-item-text" style="color: green;"><b>Response : </b>'.$key['position'].'</p></a>';
