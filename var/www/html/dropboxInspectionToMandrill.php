...
if ($_GET['hoaid']){
  ...
  $accessToken = base64_decode($dropboxQueryResult['oauth2_key']);
	...
	$query  = "SELECT FIRSTNAME,LASTNAME,COMMUNITY_ID FROM HOAID WHERE HOA_ID =".$hoaID;
	$queryResult = pg_query($query);
	$row = pg_fetch_assoc($queryResult);
	$name = $row['firstname'].' '.$row['lastname'];
	$comID = $row['community_id'];
	$query = "SELECT LEGAL_NAME FROM COMMUNITY_INFO WHERE COMMUNITY_ID=".$comID;
	$queryResult = pg_query($query);
	$row = pg_fetch_assoc($queryResult);
	$legalName = $row['legal_name'];
	$mailingData = array("key" => $m_api_key_3, "message" => array("html" => "<center><img src=\"cid:".$mandrill_cid."\" alt=\"Community Logo\"></center><br><b>Hello ".$name.", </b><br><br>Inspection deficiency, please address this issue ASAP.<br><br>Thanks,<br>Stoneridge Square Board","subject" => $subject,"from_email" => "info@stoneridgesquare.org","from_name" => $legalName,"to" => array(array("email"=>$toEmails,"name"=>$name)),"improtant"=>"true","track_opens" => "true","track_clicks" => "true","attachments" => array(array("type" => "application/pdf","name" => "attachment.pdf","content" => $fileContents)),"images"=>array( array("type" => "image/jpg","name" => $mandrill_name,"content" => $communityLogo) ),"send_at"=>"2000-01-01 00:00:00"));	
	$ch = curl_init();
