...
$message  = curl_exec($req);
	...
	if ( strpos($response, "Error in call to API function") !== false ) {
		...
		$subject = "HOA Account Statement - ".$_GET['hoaid'];
		$hoaID= $_GET['hoaid'];
	...
	$query = "SELECT LEGAL_NAME,EMAIL FROM COMMUNITY_INFO WHERE COMMUNITY_ID=".$comID;
	$queryResult = pg_query($query);
	$row = pg_fetch_assoc($queryResult);
